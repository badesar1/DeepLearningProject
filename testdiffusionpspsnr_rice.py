...
#                plt.show()
        ...
        #opt.test_noiseL = 'MR'
        outdir = '/gpfs/data/fieremanslab/Ben/DnCNN/testresults_0521_RLL'
        mdic = {"Ix": OutImg}
        sio.savemat(os.path.join(outdir, '30clean_RLL' + opt.logdir[-4:] + 'AR' + str(opt.test_noiseL) + 'denoised.mat'), mdic)
        mdic = {"Ix": NoiseImg}
        sio.savemat(os.path.join(outdir, '30clean_RLL' + opt.logdir[-4:] + 'AR' + str(opt.test_noiseL) + 'noisemap.mat'), mdic)
        mdic = {"Ix": InNoisy}
        sio.savemat(os.path.join(outdir, '30clean_RLL' + opt.logdir[-4:] + 'AR' + str(opt.test_noiseL) + 'noisyinput.mat'), mdic)
        mdic = {"Ix": Sigma}
        sio.savemat(os.path.join(outdir, '30clean_RLL' + opt.logdir[-4:] + 'AR' + str(opt.test_noiseL) + 'Sigma.mat'), mdic)
