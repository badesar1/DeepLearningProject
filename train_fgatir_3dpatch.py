...
parser.add_argument("--patchSize", type=float, default=50, help='patchsize')
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            def handle_batch():
                ...
                # results
                model.eval()
                img_train_dn = torch.clamp(imgn_train - residual, 0., 1.)
        ...
        with torch.no_grad():
            ...
            noisevar = torch.std(residual,axis=[-3,-2,-1])
            print(noisevar.size())
            ...
            #comp = torch.stack((img_train_clean[:,:,:,:,int(opt.patchSize//2)].cpu(), img_train_noisy[:,:,:,:,int(opt.patchSize//2)].cpu(), recon[:,:,:,:,int(opt.patchSize//2)].cpu(), residual[:,:,:,:,int(opt.patchSize//2)].cpu()), dim=0).view(img_train_clean.size(0),1,int(opt.patchSize),int(opt.patchSize))
            ...
            if img_train_clean.ndim == 5:
                comp[0::4,:,:,:] = img_train_clean[:,:,:,:,int(opt.patchSize//2)]
                comp[1::4,:,:,:] = img_train_noisy[:,:,:,:,int(opt.patchSize//2)]
                comp[2::4,:,:,:] = recon[:,:,:,:,int(opt.patchSize//2)]
                comp[3::4,:,:,:] = residual[:,:,:,:,int(opt.patchSize//2)]
            #comp[4::5,:,:,:] = out_train2
...
if __name__ == "__main__":
    if opt.preprocess:
        prepare_data(data_path='/gpfs/data/fieremanslab/Ben/data', patch_size=opt.patchSize, stride=10, aug_times=1, dim=3, fname=opt.fname)
    main()
