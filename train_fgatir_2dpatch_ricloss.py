...
def main():
    ...
    step = 0
    noiseL_B=[75,75] # ingnored when opt.mode=='S'
    noiseL = 75
    RAVG = []
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            def handle_batch():
                ...
                for n in range(noise.size()[0]):
                    rnoise = torch.FloatTensor(sizeN).normal_(mean=0, std=stdN[n]/255.)
                    inoise = torch.FloatTensor(sizeN).normal_(mean=0, std=stdN[n]/255.)
                ...
                noisevar = stdN/255.
                loss = criterion(out_train, img_train_Rnoise.cuda(), noisevar.cuda(), img_train_clean.cuda()) 
                ...
                img_train_dn = torch.clamp(out_train, 0., 1.)
                ...
                if step % 10 == 0:
                    ...
                    writer.add_scalars('ind losses', {'RiceLL': criterion.rice,
                    'MSE':criterion.mse}, step)
                    #writer.add_scalars('ind losses', {'MSE':criterion.mse}, step)
        ...
        with torch.no_grad():
            for k in range(len(dataset_val)):
                ...
                out_val = model(img_val_noisy.cuda())
                denoised = torch.clamp(out_val, 0., 1.)
            ...
            out_train = model(img_train_Rnoise.cuda())
            out_train = torch.clamp(out_train, 0., 1.)
            residual = out_train - img_train_Rnoise.cuda()
            ...
            recon = torch.clamp(out_train, 0., 1.)
