...
def main():
    ...
    files_source = glob.glob('/gpfs/data/fieremanslab/Ben/data/train/30_clean.nii')
    ...
    for f in files_source:
        ...
        for s in tqdm(range(Img.shape[-1])):
            ...
            snr = opt.test_noiseL
            sigma = 1./snr
            rnoise = torch.FloatTensor(ISource.size()).normal_(mean=0, std=sigma)
            inoise = torch.FloatTensor(ISource.size()).normal_(mean=0, std=sigma)
            INoisy = torch.sqrt((ISource + rnoise)**2 + inoise**2)
            #INoisy = ISource + noise
            ch2 = torch.Tensor([sigma]).view(1,1,1,1)*torch.ones_like(INoisy)
            minput = torch.cat((INoisy,ch2),dim=1)
            #ISource, INoisy = Variable(ISource), Variable(INoisy)
            with torch.no_grad(): 
                model_out = model(minput.cuda())
                #print(model_out.size())
                noisemap = model_out[0,0,:,:]
                Out = INoisy.cuda()-noisemap
...
#                plt.show()
        ...
        #opt.test_noiseL = 'MR'
        outdir = '/gpfs/data/fieremanslab/Ben/DnCNN/testresults_0421_rice'
        mdic = {"Ix": OutImg}
        sio.savemat(os.path.join(outdir, '30clean_dncnn' + opt.logdir[-4:] + 'AG' + str(opt.test_noiseL) + '2ch_denoised.mat'), mdic)
        mdic = {"Ix": NoiseImg}
        sio.savemat(os.path.join(outdir, '30clean_dncnn' + opt.logdir[-4:] + 'AG' + str(opt.test_noiseL) + '2ch_noisemap.mat'), mdic)
        mdic = {"Ix": InNoisy}
        sio.savemat(os.path.join(outdir, '30clean_dncnn' + opt.logdir[-4:] + 'AG' + str(opt.test_noiseL) + '2ch_noisyinput.mat'), mdic)
        mdic = {"Ix": Sigma}
        sio.savemat(os.path.join(outdir, '30clean_dncnn' + opt.logdir[-4:] + 'AG' + str(opt.test_noiseL) + '2ch_Sigma.mat'), mdic)
